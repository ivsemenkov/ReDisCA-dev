% FORENSIC COPY of svendaehne/matlab_SPoC SPoC/spoc.m
% commit 18e4754aec1411160fd5b7ef0db852f1e0a87d90
% SHA-256 0979006739b43b9d74e3a9321f3fc232374930b69fc69045a02d6e49173eadd2
% ONLY changes vs stock:
%   - function renamed to spoc_save_surrogates
%   - extra outputs lambda_samples, r_samples
%   - optional save of surrogate max-|lambda| distribution
% Scientific path (standardization, cov, whitening, eig, random-phase null) UNCHANGED.
% Stock file in vendor/pinned_stock_SPoC/SPoC/spoc.m is not overwritten.

function [W, A, lambda_values, p_values_lambda, Cxx, Cxxz, Cxxe, lambda_samples, r_samples] = spoc_save_surrogates(X, z, varargin)
...
lambda_samples = [];  % FORENSIC extra output
r_samples = [];       % FORENSIC extra output
...
% FORENSIC: persist surrogate max-|lambda| distribution. Not present in stock spoc.m.
if exist('FORENSIC_OUT_DIR','var') && ~isempty(FORENSIC_OUT_DIR) && exist('lambda_samples','var')
    if ~exist(FORENSIC_OUT_DIR,'dir'); mkdir(FORENSIC_OUT_DIR); end
    if ~exist('FORENSIC_SEED','var'); FORENSIC_SEED = NaN; end
    save(fullfile(FORENSIC_OUT_DIR, sprintf('spoc_surrogates_seed_%g.mat', FORENSIC_SEED)), ...
        'lambda_samples', 'r_samples', 'lambda_values', 'p_values_lambda', 'FORENSIC_SEED', '-v7.3');
end
