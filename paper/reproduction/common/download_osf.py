...
Does not re-download a file whose SHA-256 already matches the expected hash.
"""
...
# AIRI / paper MEG assets on https://osf.io/8rk67/
OSF_8RK67_FILES: dict[str, dict[str, Any]] = {
    ...
    "ibfctfprespm8_AD_run1_raw_tsss_mc.dat": {
        ...
        "dest_dir": "meg",
        "required_for": [],
        "note": "Not loaded by AIRI main script; skip by default",
    },
}
...
def download_meg_sensor_assets() -> dict[str, str]:
    ...
    for name, spec in OSF_8RK67_FILES.items():
        if "meg_sensor" in spec["required_for"]:
            path = ensure_osf_file(name, catalog=catalog)
            paths[name] = str(path)
    write_json(MEG_DIR / "downloaded.json", paths)
    return paths
...
def download_source_model_assets() -> dict[str, str]:
    ...
    paths = {}
    for name, spec in OSF_8RK67_FILES.items():
        if "source_localization" in spec["required_for"]:
            path = ensure_osf_file(name, catalog=catalog)
            paths[name] = str(path)
    write_json(SOURCE_MODEL_DIR / "downloaded.json", paths)
    return paths
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Download ReDisCA OSF assets.")
    parser.add_argument(
        "subset",
        choices=["meg-sensor", "source-models", "all-listed"],
        help="Which OSF files to fetch",
    )
    args = parser.parse_args()
    if args.subset == "meg-sensor":
        print(download_meg_sensor_assets())
    elif args.subset == "source-models":
