...
Eigenvector signs are not compared directly. Align signs first. For nearly
degenerate eigenspaces compare principal angles rather than individual axes.
"""
...
def sign_align_vectors(
    ...
    """Flip each estimated row/column vector so its inner product with reference is >= 0.

    Accepts 1-D vectors or 2-D arrays with matching shape. For 2-D, each row is
...
def principal_angles_cosines(
    ...
    """Return cosines of principal angles between column-spaces of two bases.

    ``basis_a`` and ``basis_b`` have shape ``(n_features, n_dim)``.
    """
...
def subspace_similarity(
    ...
    """Compare two row-oriented component matrices via principal angles."""
